  ...
  const handleTaskClick = (item, index) => {
    ...
    setTimeout(() => {
      setCompletedTasks((prev) => {
        const newCompletedTasks = { ...prev, [index]: true };
    ...
    >
      ...
      >
        {realTasks.map((task, taskIndex) => (
          <div
            key={taskIndex}
            className="flex mx-auto flex-col gap-3 items-center pt-5"
          >
            ...
            <div className="flex flex-col gap-2 px-4 my-6 sm:w-[80%]">
              <h1
                className="sm:text-2xl text-xl text-center"
                style={{ color: "#5c56e2" }}
              >
              ...
              <div className="flex flex-col mt-6 gap-2">
                ...
                {task?.task?.map((item, index) => (
                  <a
                    ...
                    className="no-underline"
                    onClick={() =>
                      handleTaskClick(item, taskIndex + "-" + index)
                    }
                  >
                    <div
                      className={`px-6 py-3 rounded-md flex flex-row items-center justify-between cursor-pointer ${
