...
const Tap = () => {
  ...
  const { count, incrementTap, coinsPerTap } = useTapContext();

  const [animate, setAnimate] = useState(false);
  const [energy, setEnergy] = useState(() => {
    const savedEnergy = localStorage.getItem("energy");
    return savedEnergy !== null ? parseInt(savedEnergy, 10) : 500;
  });
  ...
  useEffect(() => {
    ...
    if (savedTime) {
      ...
      setEnergy((prevEnergy) => Math.min(prevEnergy + energyGain, 500));
    }
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setEnergy((prevEnergy) => {
        const newEnergy = prevEnergy < 500 ? prevEnergy + 1 : 500;
