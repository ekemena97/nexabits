...
const Tap = () => {
  ...
  const { count, incrementTap, coinsPerTap, energyLimit } = useTapContext(); // Get energyLimit from context
  ...
  const [showDailyCheckIn, setShowDailyCheckIn] = useState(false); // State to manage DailyCheckIn visibility

  useEffect(() => {
    const savedTime = localStorage.getItem("lastUpdateTime");
    if (savedTime) {
      const elapsedSeconds = Math.floor((Date.now() - parseInt(savedTime, 10)) / 1000);
      const energyGain = Math.floor(elapsedSeconds / 0.6); // 1 energy per 0.6 seconds
  ...
  const handleClick = () => {
    if (isTappable) {
      ...
      setEnergy((prevEnergy) => {
        const newEnergy = Math.max(prevEnergy - coinsPerTap, 0);
        localStorage.setItem("energy", newEnergy);
        localStorage.setItem("lastUpdateTime", Date.now());
        return newEnergy;
      });
  ...
  const handleGiftClick = () => {
    ...
    setShowDailyCheckIn(true); // Show DailyCheckIn component
    setTimeout(() => setShowGiftIcon(true), 24 * 60 * 60 * 1000); // Reappear after 24 hours
  };
  ...
  return (
    <section className={`h-full w-[90%] flex flex-col mt-20 mb-24 relative`}>
      ...
      >
        ...
        {showDailyCheckIn && <DailyCheckIn onClose={() => setShowDailyCheckIn(false)} />} {/* Add DailyCheckIn component */}
