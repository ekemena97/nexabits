...
const Referrals = () => {
  ...
  useEffect(() => {
    if (userId) {
      ...
      // Save data to Firestore if it doesn't already exist
      const saveDataToFirestore = async () => {
        try {
          const checkResponse = await fetch(`${process.env.REACT_APP_API_URL}/user?userId=${userId}`);
          const checkData = await checkResponse.json();
          
          if (checkData.exists) {
            console.log('User data already exists, not updating the date.');
            return;
          }
      ...
      const saveReferralToFirestore = async () => {
        try {
          const checkResponse = await fetch(`${process.env.REACT_APP_API_URL}/referral?userId=${userId}&referrerId=${referrerId}`);
          const checkData = await checkResponse.json();

          if (checkData.exists) {
            console.log('Referral data already exists, not updating the date.');
            return;
          }
  ...
  }, [userId, referrerId, firstName, username, isBot, isPremium]);
